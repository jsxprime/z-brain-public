...
export const getModel = (takeModel?: string) => {
  ...
  // OpenRouter: route through OpenRouter using the native Anthropic provider
  // (preserves structured output/tool-use schema enforcement)
  if (provider === "openrouter" || getDefaultChatProviderType() === "openrouter") {
    const openrouterKey = resolveApiKey("openrouter");
    if (!openrouterKey) {
      throw new Error("OpenRouter provider selected but OPENROUTER_API_KEY is missing.");
    }
